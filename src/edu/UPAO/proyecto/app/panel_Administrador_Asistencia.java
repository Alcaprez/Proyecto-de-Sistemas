...
public class panel_Administrador_Asistencia extends javax.swing.JPanel {
    ...
    private void construirInterfazModerna() {
        ...
        // --- AGREGA ESTO PARA MEJORAR LA VISUALIZACIÓN DEL NOMBRE ---
        jTable1.getColumnModel().getColumn(0).setPreferredWidth(250); // Hacemos la columna Nombre más ancha
        jTable1.getColumnModel().getColumn(1).setPreferredWidth(100); // Rol
    ...
    private void cargarDatosDesdeBD() {
        ...
        if (datosEnMemoria != null) {
            // El índice 1 corresponde a la columna "Rol" según tu modelo de tabla
            datosEnMemoria.removeIf(fila -> 
                fila[1] != null && fila[1].toString().equalsIgnoreCase("Gerente")
            );
        }
    ...
    private void calcularKPIsLocales() {
        int total = datosEnMemoria.size();
        int presentes = 0;
        int tardanzas = 0;
        int ausencias = 0;

        for (Object[] fila : datosEnMemoria) {
            // El índice 5 es la columna "Estado"
            String estado = (fila[5] != null) ? fila[5].toString().toUpperCase() : "";

            if (estado.contains("ASISTIO") || estado.equals("1") || estado.contains("PRESENTE")) {
                presentes++;
            } else if (estado.contains("TARDANZA") || estado.equals("2") || estado.contains("TARDE")) {
                tardanzas++;
            } else if (estado.contains("FALTA") || estado.equals("3") || estado.contains("AUSENTE")) {
                ausencias++;
            }
        }

        // Actualizamos las etiquetas del sidebar
    ...
    private void filtrarTablaLocalmente() {
        ...
        for (Object[] fila : datosEnMemoria) {
            ...
            String rol = (fila[1] != null) ? fila[1].toString() : "";
            String estadoBD = (fila[5] != null) ? fila[5].toString() : "PENDIENTE";
