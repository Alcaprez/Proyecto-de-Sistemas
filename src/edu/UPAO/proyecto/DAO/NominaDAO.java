...
public class NominaDAO {
    ...
    // 1. LISTAR EMPLEADOS CON SU ESTADO DE PAGO (PAGADO / PENDIENTE)
    public List<Object[]> listarNominaMes(int idSucursal, String mes, int anio, String textoBusqueda) {
        ...
        String sql = "SELECT e.id_empleado, p.nombres, p.apellidos, e.rol, e.sueldo, " +
                     // Subconsulta inteligente: Verifica si existe el pago en ese mes/año
        ...
        try (Connection cn = new Conexion().establecerConexion();
            ...
            while (rs.next()) {
                lista.add(new Object[]{
                    ...
                    rs.getString("rol"),
                    rs.getDouble("sueldo"),
                    rs.getString("estado_pago"), // Columna calculada
                    rs.getTimestamp("fecha_pago") // Puede ser null si es PENDIENTE
                });
            }
    ...
    private int obtenerCajaAbiertaEnSucursal(int idSucursal) {
        ...
        try (Connection cn = new Conexion().establecerConexion();
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next()) return rs.getInt("id_caja");
        } catch (Exception e) {}
        return -1;
    }
}